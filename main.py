...
thread_id = "thread_JTdCfxHNWlZkkKsghito15MK"
assis_id = "asst_6cNICNjcpbNXoD8911SZkbZF"
...
# Create sidebar where users can upload files
file_uploaded = st.sidebar.file_uploader(
    "Upload a file to be transformed into embeddings", key="file_upload"
)

# Create upload file button to store the File ID
...
def process_message_with_citations(message):
    ...
    for index, annotation in enumerate(annotations):
        ...
        if file_citation := getattr(annotation, "file_citation", None):
            ...
            citations.append(
                f'[{index + 1}] Click [here](https://muse.jhu.edu/pub/1/article/858044/pdf) to download {cited_file["filename"]}'
...
# Check that chat is not already in session
if st.session_state.start_chat:
    ...
    if prompt := st.chat_input("What's new?"):
        ...
        with st.spinner("Wait... Generating response..."):
            ...
            # Process and display assistant messages with list
        ...
        st.write(
            "Please upload at least one file to get started by clicking on the 'Start Chat' button"
        )
